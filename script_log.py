...
def read_and_replace_content(file_path, name, steps,logs):
    ...
    try:
        ...
        test_name = find_test_key(name[0])

        if not test_name:
            raise ValueError(f"Test name '{name[0]}' not found in mapper.")

        tag = f"<testdoc:{test_name}></testdoc:{test_name}>"

        # Replace the tag with steps
        updated_content = content.replace(tag, "\n".join(str(step) for step in steps))
        updated_content = updated_content + "Logs for the test\n" +"\n".join(str(log) for log in logs)

        # Remove other specific tags
        updated_content = re.sub(r'<testdoc:[a-zA-Z0-9_-]+>', '', updated_content)
        updated_content = re.sub(r'</testdoc:[a-zA-Z0-9_-]+>', '', updated_content)
...
if __name__ == "__main__":
    try:
        with open("logs_example1.txt", "r") as file:
        ...
        if not name or not steps:
            ...
            read_and_replace_content("ref.md", name, steps,logs)

    except FileNotFoundError:
        print("Log file 'logs_example.txt' not found.")
    except Exception as e:
        print(f"An error occurred: {e}")
